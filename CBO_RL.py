...
def Q_SGD_gen(update_step):
    def algo(Q_net=Net(), M=1000, epochs=100, γ=0.9, τ=0.01, S=S, A_idx=A_idx, R=R, a_s=a_s, π=π, σ=σ, ϵ=ϵ, Q_net_comp=None, n=100):
        Rem = torch.tensor([])
        N = S.size()[0]
        if V_net_comp:
            x_ls = torch.linspace(0,2*np.pi,n+1)[:-1].view(-1,1)
        ...
        for k in trange(epochs, leave=False, position=0, desc="Epoch"):
            B, Rem = gen_batches(N-2,M, Rem)
            for i,B_θ in enumerate(tqdm(B,leave=False, position=0,desc="Batch")): 
                update_step(Q_net, γ, τ, S, A_idx, R, a_s, B_θ, π=π, σ=σ, ϵ=ϵ)                
                with torch.no_grad():
...
def Q_eval_UR_SGD_update_step(Q_net, γ, τ, S, A_idx, R, a_s, B_θ, π=π, σ=σ, ϵ=ϵ):     
    s = S[B_θ]
    r = R[B_θ].view(-1)
    ...
    Q_net.zero_grad()
    ĵ.backward(j)
def Q_eval_DS_SGD_update_step(Q_net, γ, τ, S, A_idx, R, a_s, B_θ, π=None, σ=None, ϵ=None):     
    s = S[B_θ]
    r = R[B_θ].view(-1)
    a_idx = A_idx[B_θ].type(torch.LongTensor).view(-1)
    s_1 = S[B_θ+1]
...
def Q_eval_BFF_G_SGD_update_step(Q_net, γ, τ, S, A_idx, R, a_s, B_θ, π=None, σ=None, ϵ=None):     
    s = S[B_θ]
    r = R[B_θ].view(-1)
    a_idx = A_idx[B_θ].type(torch.LongTensor).view(-1)
    s_1 = S[B_θ+1]
...
def Q_eval_BFF_L_SGD_update_step(Q_net, γ, τ, S, A_idx, R, a_s, B_θ, π=None, σ=None, ϵ=None):     
    s = S[B_θ]
    r = R[B_θ].view(-1)
    a_idx = A_idx[B_θ].type(torch.LongTensor).view(-1)
    s_1 = S[B_θ+1]
...
# %%
Q_eval_UR_SGD_star = Q_SGD_gen(Q_eval_UR_SGD_update_step)(S=S_long, R=R_long, M=1000, epochs=1, τ=0.01)
...
def plotQ(Q_s, e_s, lb_s, Q_star, lb_star):
    ...
    x_l = torch.linspace(0,2*np.pi,100)
