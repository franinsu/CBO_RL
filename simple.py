...
def L(x):
    return torch.exp(torch.sin(2*x**2))+0.01*torch.mean((x-x_hat-np.pi/2)**2)
# %%
...
def CBO(M=10, N=30, τ=0.1, σ=2,
        γ=0.9, λ=1., δ=1e-3,
             τ_k=lambda k: 0.1, η_k=lambda k: 0.5, β_k=lambda k: 10):
    with torch.no_grad():
        x = torch.rand(N)*σ
        ...
        for i in range(M):
            ...
            x += (-λ*η+τ*np.sqrt(η)*torch.normal(z))*(x-x̄)
            L_x = torch.tensor([L(x_i) for x_i in x])
...
η_k=lambda k: 0.1
β_k=lambda k: 5
e_ls_CBO, L_ls_CBO, x_ls_CBO, ē_ls_CBO, L̄_ls_CBO, x̄_ls_CBO = CBO(M=M,N=N,τ=τ,σ=σ,γ=γ,λ=λ,δ=δ,τ_k=τ_k,η_k=η_k,β_k=β_k)
...
for i in range(N):
    pltRun(x_ls_CBO[:,i], L_ls_CBO[:,i],e_ls_CBO[:,i], "C2", M, r"$x_{CBO}$", "C2")
