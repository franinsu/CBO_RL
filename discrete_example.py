...
def get_parameters():
    return {
        ...
        "n_s": 32,
        "s_s": torch.arange(32)*2*np.pi/32,
    }
...
def resample_save_policy(suffix="tabular", l=int(1e6),l_long=int(1e7)):
    params = get_parameters()
    s_0, σ, ϵ, a_s, s_s, n_s = [params[p] for p in ["s_0", "σ", "ϵ", "a_s", "s_s", "n_s"]]
    S, A_idx = sample_policy_discrete(s_0, π, σ, ϵ, a_s, l, s_s, n_s, verbose=True)
    R = r(s_s[S])
