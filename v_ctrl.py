...
#     axs[i].set_title(s)
# def η_k(k): return max(0.1*0.9992**k, 0.075)
# def τ_k(k): return max(1*0.9985**k,0.8*0.9992**k, 0.3)
# def β_k(k): return min(8*1.002**k,20)
# for i,(f,s) in enumerate(zip([η_k, τ_k, β_k], ["η_k","τ_k","β_k"])):
#     axs[i].plot(k_s, [f(k) for k in k_s])
#     axs[i].set_title(s)
...
def run_CBO_all():
    Q_ctrl_UR_CBO, e_ctrl_UR_CBO = Q_CBO_gen(Q_ctrl_UR_CBO_L)(
        S, A_idx, R, a_s, π, σ, ϵ,
        N=N, m=m, epochs=epochs, τ_k=τ_k, η_k=η_k, β_k=β_k, δ=δ, Q_net_comp=Q_ctrl_UR_SGD_star,early_stop=early_stop
    )
    ...
    return (Q_ctrl_UR_CBO, Q_ctrl_DS_CBO, Q_ctrl_BFF_CBO), (e_ctrl_UR_CBO, e_ctrl_DS_CBO, e_ctrl_BFF_CBO)
#%%
...
for j,lb in enumerate(["UR","DS", "BFF"]):
    for k in range(a_n):
        all_data = all_data.append(pd.DataFrame({"i":0,
                                                            "x":x_s.detach().numpy(),
...
for i in range(n_runs):
    for r, algo in [(run_SGD_all, "SGD"), (run_CBO_all, "CBO")]:
        ...
        for j,lb in enumerate(["UR","DS", "BFF"]):
            for k in range(a_n):
                all_data = all_data.append(pd.DataFrame({"i":i,
                                                        "x":x_s.detach().numpy(),
...
#%%
g = sns.FacetGrid(data = all_data,col="plot", row="sampling", hue="algo", sharex=False,sharey=False, palette = ["black","C0","C1"], margin_titles=True)
...
g.add_legend(title="")
plt.savefig("figs/Q_ctrl_SGD_vs_CBO_summary_3.png")
# %%
# N = 90
# m = 1000
# epochs = 1
